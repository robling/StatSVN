...
public class SvnInfoUtils implements ISvnInfoProcessor {
	static Pattern patternPercent = Pattern.compile("%(?![0-9a-fA-F]{2})");
	static Pattern patternAdd = Pattern.compile("\\+");

	// HACK: we "should" parse the output and check for a node named root, but this
	// will work well enough
	private static final String SVN_INFO_WITHREPO_LINE_PATTERN = ".*<root>.+</root>.*";
	static Pattern pattern_SVN_INFO_WITHREPO_LINE_PATTERN= Pattern.compile(SVN_INFO_WITHREPO_LINE_PATTERN);
	...
	protected static class SvnInfoHandler extends DefaultHandler {
			...
			if (isRootFolder && eName.equals("url")) {
				...
				if (sCurrentRevision == null || sCurrentUrl == null || sCurrentKind == null) {
					throw new SAXException("Invalid svn info xml; unable to find revision or url for path ["
							+ sCurrentPath + "]" + " revision=" + sCurrentRevision + " url:" + sCurrentUrl + " kind:"
							+ sCurrentKind);
				}
			...
			if (eName.equals("entry")) {
				...
				if (!isValidInfoEntry(attributes)) {
					throw new SAXException(
							"Invalid svn info xml for entry element. Please verify that you have checked out this project using "
				...
				if (!isValidCommit(attributes)) {
					throw new SAXException(
							"Invalid svn info xml for commit element. Please verify that you have checked out this project using "
									+ "Subversion 1.3 or above, not only that you are currently using this version.");
				}
		...
		/**
		 * Is this a valid info entry? Check to see if we can read path, kind and
		 * revision.
		 * 
		 * @param attributes the xml attributes.
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sf.statsvn.util.ISvnInfoProcessor#absoluteToRelativePath(java.lang.
	 * String)
	 */
	 ...
	 * 
	 * @see
	 * net.sf.statsvn.util.ISvnInfoProcessor#absolutePathToUrl(java.lang.String)
	 */
	 ...
	 * 
	 * @see
	 * net.sf.statsvn.util.ISvnInfoProcessor#relativePathToUrl(java.lang.String)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sf.statsvn.util.ISvnInfoProcessor#relativeToAbsolutePath(java.lang.
	 * String)
	 */
	 ...
	 * 
	 * @see
	 * net.sf.statsvn.util.ISvnInfoProcessor#existsInWorkingCopy(java.lang.String)
	 */
		...
		if (sModuleName == null) {
			...
			if (getRootUrl().length() < getRepositoryUrl().length() || getRepositoryUrl().length() == 0) {
				...
				sModuleName = "";
					...
					sModuleName = urlDecode(getRootUrl().substring(getRepositoryUrl().length()), "UTF-8");
			}
	 ...
	 * 
	 * @see
	 * net.sf.statsvn.util.ISvnInfoProcessor#getRevisionNumber(java.lang.String)
	 */
	 ...
	 * Invokes svn info.
	 * 
	 * @param bRootOnly true if should we check for the root only or false otherwise
	 ...
	 * 
	 * @param bRootOnly true if we need the root only
	 ...
	 * 
	 * @param bRootOnly load only the root?
	 * @throws LogSyntaxException if the format of the svn info is invalid
	 * @throws IOException        if we can't read from the response stream.
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sf.statsvn.util.ISvnInfoProcessor#loadInfo(net.sf.statsvn.util.
	 * ProcessUtils)
	 */
	 ...
	 * 
	 * @see
	 * net.sf.statsvn.util.ISvnInfoProcessor#urlToAbsolutePath(java.lang.String)
	 */
	public String urlToAbsolutePath(String url) {
		...
		String decoded;
		try {
			decoded = urlDecode(result, "UTF-8");
	 ...
	 * 
	 * @see
	 * net.sf.statsvn.util.ISvnInfoProcessor#urlToRelativePath(java.lang.String)
	 */
	...
	/**
	 * Verifies that the "svn info" command can return the repository root (info
	 * available in svn >= 1.3.0)
	 * 
	 * @throws SvnVersionMismatchException if <tt>svn info</tt> failed to provide a
	 *                                     non-empty repository root
	 */
	public synchronized void checkRepoRootAvailable() throws SvnVersionMismatchException {
		...
		try {
			...
			while (reader.hasNextLine()) {
				...
				if (pattern_SVN_INFO_WITHREPO_LINE_PATTERN.matcher(line).matches()     ) {
					// We have our <root> element in the svn info AND it's not empty --> checkout
					// performed
	...
	/**
	 * Deal with the urldecode with <b>%</b> and <b>+</b>,which will cause:
	 * <br/><i>java.lang.IllegalArgumentException: URLDecoder: Illegal hex characters in escape (%) pattern</i>
	 * @param srcStr
	 * @param enc
	 * @return
	 */
	public static String urlDecode(String srcStr, String enc) {
		Matcher matcher = patternPercent.matcher(srcStr);
		String src_tmp = matcher.replaceAll("%25");
		matcher = patternAdd.matcher(src_tmp);
		src_tmp = matcher.replaceAll("%2B");
		String decoded=null;
		try {
			  decoded = URLDecoder.decode(src_tmp, "UTF-8");
			  return decoded;
		} catch (Exception e) {
			SvnConfigurationOptions.getTaskLogger().error("ERROR,src:"+srcStr);
